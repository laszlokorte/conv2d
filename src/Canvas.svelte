<script>
	...
	$: if(image.values==null||image.values.length != image.size.y || image.values[0].length != image.size.x) {
	...
	function moveDrawing(evt) {
		...
		if(brush.prev.x == null) {
		  ...
		  const deltaY = Math.abs(intY - brush.prev.y)
			...
			if (m <= 1) for (let i=0; i <= deltaX; i++) placeBrush( Math.ceil(brush.prev.x + i * signX), Math.ceil(brush.prev.y + i * m * signY), fracX, fracY );
			else for (let i=0; i<= deltaY; i++) placeBrush( Math.ceil(brush.prev.x + i / m * signX), Math.ceil(brush.prev.y + i * signY), fracX, fracY );
		}
...
<dd>
	<ul>
	<li><label><input type="radio" bind:group={selectedRange} value={0} /> Positive (0.0 to 1.0)</label></li>
	<li><label><input type="radio" bind:group={selectedRange} value={-1} /> Symmetric (-1.0 to 1.0)</label></li>
</ul>
</dd>
<dt></dt>
